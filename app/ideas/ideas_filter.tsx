...
export default function IdeasFilter({ ideas }: { ideas: IdeaRow[] }) {
  ...
  const input_class = "bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-faint)] focus:border-[var(--text-muted)] focus:outline-none transition-colors";
  ...
  return (
    <>
      ...
      {/* results */}
      {filtered.length === 0 ? (
        <div className="border border-[var(--border-primary)] rounded-lg p-12 text-center">
        ...
        <div className="space-y-4">
          {filtered.map((idea, i) => (
            <a
              key={idea.id}
              href={`/ideas/${idea.id}`}
              className="block border border-[var(--border-primary)] rounded-lg p-6 hover:border-[var(--border-secondary)] transition-colors group"
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-xs text-[var(--text-faint)]">per month pledged</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 text-xs text-[var(--text-faint)]">
                <span className="text-[var(--text-secondary)]">&#9650; {Number(idea.upvote_count)}</span>
                <span className="text-[var(--text-separator)]">·</span>
                <span>
                  <span className="text-[var(--text-secondary)]">{idea.pledge_count}</span>{" "}
                  {Number(idea.pledge_count) === 1 ? "sponsor" : "sponsors"}
                </span>
                <span className="text-[var(--text-separator)]">·</span>
                <span>
                  submitter asks{" "}
                  <span className="text-[var(--text-secondary)]">${idea.monthly_ask}</span>/mo
                </span>
                <span className="text-[var(--text-separator)]">·</span>
                <span
                  className={`border rounded px-1.5 py-0.5 ${STATUS_COLORS[idea.status] ?? "text-gray-500 border-gray-400"}`}
                >
                  {STATUS_LABELS[idea.status] ?? idea.status}
                </span>
                <span className="text-[var(--text-separator)]">·</span>
                <span className="border border-blue-600 text-blue-600 rounded px-1.5 py-0.5">
