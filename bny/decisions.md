...
2026-02-22 — feature 020 (public user profiles) complete. profiles table (id refs auth.users, display_name, bio, website) with RLS (public read, owner upsert/update). /profile/[id] shows display name (email prefix fallback via service role), bio, website, member since, ideas list with pledge/upvote counts. /dashboard/profile edit form with useActionState, upsert on save, validation (name 50 chars, bio 500, website URL format). dashboard links to edit profile + view public profile. migration applied via Management API.
